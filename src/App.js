...
function App() {
  ...
  //     if (res && res.categoryList && res.categoryList.length > 0) {
  //       setCategoryData(res.categoryList);

  //       // Tạo mảng danh mục con
  //       const subCatArr = [];

  //       // Lặp qua danh mục để lấy danh mục con
  //       res.categoryList.forEach((cat) => {
  //         if (cat.children && cat.children.length > 0) {
  //           subCatArr.push(...cat.children); // Dùng spread operator để đẩy tất cả danh mục con vào mảng
  //         }
  //       });

  //       // Cập nhật state danh mục con
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res && res.categoryList && res.categoryList.length > 0) {
          ...
          // Tạo mảng danh mục con
          const subCatArr = res.categoryList.flatMap((cat) =>
            cat.children || [] // Dùng flatMap để đẩy tất cả danh mục con vào mảng
          );
    ...
    // Lấy thông tin quốc gia từ localStorage
    const location = localStorage.getItem("location");
    if (location) {
      setselectedCountry(location);
    } else {
